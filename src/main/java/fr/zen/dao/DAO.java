...
/**
 * Simply maps the entitymanager. It simplifies refactoring and wraps some logic (limited queries).
 * 
 */
@Named
@Lock(LockType.READ)
public class DAO {
	...
	public <E> List<E> namedFind(Class<E> clazz, String query, int min, int max, String search) {
		Query q = em.createNamedQuery(query, clazz);
		q.setParameter("search", search);
		return queryRange(q, min, max).getResultList();
	}
